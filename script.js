document.addEventListener('DOMContentLoaded', () => {
    // --- Page Transitions Logic (START) ---
    const transitionContainer = document.getElementById('transition-container') || document.body;
    console.log("Page Loaded, checking for transitions...");
    
    // 1. Initial Entry Animation
    const prevTransition = sessionStorage.getItem('pageTransition');
    console.log("Previous transition direction:", prevTransition);
    
    if (prevTransition === 'to-right') {
        transitionContainer.classList.add('page-transition-enter-right');
    } else if (prevTransition === 'to-left') {
        transitionContainer.classList.add('page-transition-enter-left');
    }
    ...
    const navArrows = document.querySelectorAll('.glass-arrow');
    navArrows.forEach(arrow => {
        arrow.addEventListener('click', (e) => {
            ...
            const isRight = arrow.closest('.floating-arrow-right');
            const direction = isRight ? 'to-right' : 'to-left';
            
            console.log("Navigation triggered:", direction, "to", href);

            // Apply exit animation
            document.body.classList.add('transitioning');
            if (isRight) {
                transitionContainer.classList.add('page-transition-exit-left');
            } else {
                transitionContainer.classList.add('page-transition-exit-right');
            }

            // Save direction for next page enter animation
